...
public class Game {
    ...
    private List<Door> doorList = new ArrayList<>(Arrays.asList(doorArr)); // initially {Door.ONE, Door.TWO, Door.THREE})); Removed from this list will be: (1) the contestant-picked door and (2) the winning door.
    ...
    boolean originalChoiceWins; // true if not switching wins
    boolean switchWins; // true if switching wins
    ...
    Game(int gameNumber) {
        ...
        shownDoor = getShownDoor(doorList); // After contestant picks a door Monty is assumed to always open a goat door and give the contestant the opportunity to switch to the other unopened door.  In reality, Monty did not always give the contestant the chance to make this switch.
    ...
    private static Door getShownDoor(List<Door> doorList) {
        Random r = new Random();
